...
namespace dak::tantrix
{
   ...
   std::vector<solver::sub_problem_t::ptr_t> any_shape_puzzle_t::create_initial_sub_problems() const
   {
   ...
   std::vector<solver::sub_problem_t::ptr_t> any_shape_puzzle_t::create_sub_problems(
         ...
         const solver::solution_t::ptr_t& /*a_partial_solution*/) const
   {
      ...
      {
         if (!current_sub_puzzle->other_tiles[0].has_color(color))
            continue;
         ...
         {
            if (!current_sub_puzzle->other_tiles[i].has_color(color))
               break;

            sub_puzzle_t sub_puzzle(*current_sub_puzzle);
            sub_puzzle.tile_to_place = current_sub_puzzle->other_tiles[i];
            sub_puzzle.other_tiles.erase(sub_puzzle.other_tiles.begin() + i);
            sub_puzzle.right_sub_puzzles_count -= 1;
            subs.emplace_back(std::make_shared<sub_puzzle_t>(sub_puzzle));
         }
         break;
      }
   ...
   std::vector<solver::solution_part_t::ptr_t> any_shape_puzzle_t::get_sub_problem_potential_parts(
         const solver::sub_problem_t::ptr_t& a_current_sub_problem,
         const solver::solution_t::ptr_t& a_partial_solution) const
   {
      auto current_sub_puzzle = std::dynamic_pointer_cast<sub_puzzle_t>(a_current_sub_problem);
      ...
      if (current_sub_puzzle->tile_to_place.has_color(first_color))
      {
         if (partial_solution->tiles_count() == 0) {
            next_positions.emplace_back(
               placed_tile_t::make(
                  current_sub_puzzle->tile_to_place, position_t(0, 0)));
         ...
         for (int i = 0; i < 6; ++i)
         {
            ...
            for (int rotation = 0; rotation < 6; ++rotation) {
               next_positions.emplace_back(
                  placed_tile_t::make(
                     current_sub_puzzle->tile_to_place.rotate(rotation), new_pos));
            }
      ...
      else
      {
         for (size_t i = 1; i < my_line_colors.size(); ++i)
         {
            ...
            if (current_sub_puzzle->tile_to_place.has_color(color))
            {
               ...
               const size_t tiles_count = current_sub_puzzle->count_tiles_of_color(color);
